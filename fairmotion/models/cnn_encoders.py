...
class LSTMEncoder(nn.Module):
    def __init__(
        ...
        if not lstm:
            ...
            p = (d * (k - 1)) // 2
            dropout = 0.2
            self.conv1 = weight_norm(nn.Conv1d(in_channels=input_dim, out_channels=input_dim, kernel_size=k, dilation = d, padding=p))
            self.relu1 = nn.ReLU()
            self.dropout1 = nn.Dropout(dropout)

            self.conv2 = weight_norm(nn.Conv1d(in_channels=input_dim, out_channels=input_dim, kernel_size=k, dilation = d, padding=p))
            self.relu2 = nn.ReLU()
            self.dropout2 = nn.Dropout(dropout)

            self.conv3 = weight_norm(nn.Conv1d(in_channels=input_dim, out_channels=input_dim, kernel_size=k, dilation=d, padding=p))
            self.relu3 = nn.ReLU()
    ...
    def forward(self, input):
        ...
        '''
        init_input3 = input
        input = self.conv3(input)  # (n, dim, len)
        input = self.relu3(input)
        # input = F.glu(input, dim=1)
        input = self.dropout3(input)
        input = self.relu(init_input3 + input)
        '''
